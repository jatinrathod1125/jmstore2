...
<div class="row">
    ...
    <div class="col-12 mb-4">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">{{ isset($banner) ? 'Edit Banner' : 'Create Banner' }}</h4>

                <form id="bannerForm" method="POST" action="{{ isset($banner) ? route('admin.banners.update', $banner) : route('admin.banners.store') }}">
                    @csrf
                    @if(isset($banner))
                        @method('PUT')
                    @endif
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input name="title" class="form-control" placeholder="Banner title" value="{{ old('title', $banner->title ?? '') }}" required>
                    </div>

                    <input type="hidden" name="html" id="html" value="{{ old('html', $banner->html ?? '') }}">
                    <input type="hidden" name="css" id="css" value="{{ old('css', $banner->css ?? '') }}">

                    <div class="d-flex flex-row gap-2">
                        <button type="submit" class="btn btn-primary btn-sm">
                            <i class="fa fa-save"></i> {{ isset($banner) ? 'Update Banner' : 'Save Banner' }}
                        </button>
...
(function() {
    ...
    const isEditMode = {{ isset($banner) ? 'true' : 'false' }};
    const existingHtml = @json($banner->html ?? '');
    const existingCss = @json($banner->css ?? '');
...
`;
    ...
    // Default content for new banners
    const defaultHtml = `
    ...
    window.addEventListener('load', function() {
        setTimeout(function() {
            ...
            try {
                ...
                if (isEditMode && existingHtml && existingCss) {
                    console.log('Loading existing banner content...');
